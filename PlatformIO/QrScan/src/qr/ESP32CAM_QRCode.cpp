...
void ESPino32QRCode::init(ESP32CAM *cam)
{
	cam_ = *cam;
...
void dump_cells(const struct quirc_code *code) {
	...
	Serial.printf("    %d cells, corners:", code->size);
	for (u = 0; u < 4; u++)
		Serial.printf(" (%d,%d)", code->corners[u].x, code->corners[u].y);
	Serial.printf("\n");
